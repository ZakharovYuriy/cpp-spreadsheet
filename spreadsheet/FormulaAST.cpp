...
public:
	...
	void PrintFormula(std::ostream &out, ExprPrecedence parent_precedence,
		...
		bool parens_needed = PRECEDENCE_RULES[parent_precedence][precedence]
				& mask;
		if (parens_needed) {
			out << '(';
		}
...
public:
	...
	void DoPrintFormula(std::ostream &out, ExprPrecedence precedence) const
			override {
	...
	double Evaluate(const SheetInterface &sheet) const override {
		...
		case Subtract:
			...
			return left - right;
		case Multiply:
			if ((left!=0 && std::abs(max / left) < std::abs(right)) || (right !=0 && std::abs(max / right) < std::abs(left))) {
				throw FormulaError(FormulaError::Category::Div0);
			}
			return left * right;
		case Divide:
...
public:
	...
	double Evaluate(const SheetInterface &sheet) const override {
		...
		auto get_cell = sheet.GetCell(*cell_);
		CellInterface::Value result = 0.0;
		if (get_cell != nullptr) {
			result = sheet.GetCell(*cell_)->GetValue();
		}
		...
		if (std::holds_alternative<std::string>(result)) {
			...
			for (char ch : std::get<std::string>(result)) {
				if (ch < '0' || ch > '9') {
...
public:
	...
	void visitErrorNode(antlr4::tree::ErrorNode *node) override {
		throw ParsingError(
				"Error when parsing: " + node->getSymbol()->getText());
	}
